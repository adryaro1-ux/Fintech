...
class SistemaHorizontes(ctk.CTk):
    ...
    def bucle_principal(self):
        fallos_deteccion = 0
        while True:
            # CONDICIÓN CRÍTICA: Se cierra si Avaya no se detecta tras varios intentos (periodo de gracia)
            if not esta_corriendo_avaya():
                fallos_deteccion += 1
                if fallos_deteccion >= 5: # ~10 segundos de margen
                    print("Avaya no detectado tras varios intentos. Cerrando Fintech...")
                    os._exit(0)
            else:
                fallos_deteccion = 0 # Resetear si se vuelve a detectar

            if self.motor_activo and not self.ventana_abierta:
                try:
                    # LOCALIZACIÓN PRECISA: Escaneamos solo la ventana de Avaya
                    rect = obtener_rect_avaya()
                    if rect:
                        captura = pyautogui.screenshot(region=rect)
                        texto = pytesseract.image_to_string(captura, config='--psm 3')
                        
                        # REGEX ACTUALIZADA: Basada en los códigos de área específicos de Venezuela
                        # Buscamos: 212, 24[1-69], 26[123567], 251, 235, 28[568], 27[46]
                        # Seguidos de 7 dígitos.
                        regex_vzla = r'\b(212|24[1-69]|251|26[123567]|27[46]|235|28[568])\s?(\d{3})\s?(\d{4})\b'
                        search = re.search(regex_vzla, texto)
                        
                        if search:
                            # Combinamos los grupos capturados para ignorar espacios intermedios
                            num_limpio = search.group(1) + search.group(2) + search.group(3)
                            
                            tel_full = "0" + num_limpio # Agregamos el cero para la validación en Registros
                            print(f"Detectado: {tel_full} en Avaya")
