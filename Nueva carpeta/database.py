...
class Database:
    def __init__(self):
        # La base de datos se guardará en una carpeta dedicada para orden
        self.base_dir = os.path.dirname(os.path.abspath(sys.argv[0]))
        self.db_folder = os.path.join(self.base_dir, "database")
        
        # Crear la carpeta si no existe
        if not os.path.exists(self.db_folder):
            os.makedirs(self.db_folder, exist_ok=True)
            
        self.db_path = os.path.join(self.db_folder, "Fintech_horizontes.db")
        self.init_db()
    ...
    def obtener_estudios(self):
        try:
            conn = self.get_connection()
            cursor = conn.cursor()
            # Ajustamos la consulta para SQLite (JOIN con la tabla registros)
            cursor.execute('''
                SELECT e.id, e.nombre_estudio, IFNULL(a.nombre, e.clinica_emisor), e.precio, 
                       strftime('%d/%m/%Y %H:%M', e.fecha_escaneo)
                FROM estudios e LEFT JOIN registros a ON e.clinica_emisor = a.telefono
                ORDER BY e.id DESC
            ''')
            res = cursor.fetchall()
            conn.close()
